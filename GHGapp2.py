...
if file1 is not None:
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Bar(x=df1['Month'], y=totallpgco2e, name="Scope 1"))
    fig2.add_trace(go.Bar(x=df1['Month'], y=scope2, name="Scope 2"))
    fig2.add_trace(go.Bar(x=df1['Month'], y=scope3co2e, name="Scope 3"))
    fig2.update_xaxes(type='category')
    fig2.update_layout(barmode='stack',
                       title="Total emission CO2 Equivalence")

    fig3 = go.Figure(data=[go.Pie(labels=["Scope 1", "Scope 2", "Scope 3"], 
                                  values=[sumscope1, sumscope2, sumscope3])])
    fig3.update_layout(title="Total GHG by scope")

    fig4 = go.Figure(data=go.Scatter(x=df1['Month'], y=alltotalco2e))
